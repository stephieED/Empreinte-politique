...
| `interventions[]` | Speeches, questions (`type_detail`). An entry carrying `collecte: "theme_seul"` (#657) was collected **without its verbatim**: its heavy fields are **absent, never `null`** — a `"texte": null` would read as a fact about the person, where the fact is about the run (§2 rule 5). `collecte` is a closed value (`KNOWN_COLLECTES_INTERVENTION`); its **absence** is the full form. See `docs/decisions/collecte-interventions-reduite-au-theme-657.md`. A question au gouvernement is **one act** (`type_detail: "question"`, `sous_type: "QG"`, from questions.an) and **its speaking turns** (`type_detail: "question_gouvernement"`, Syceron); a turn whose act is published in the same profile, confirmed by the theme, carries `question_ref` — the act's `intervention_id`. **Derived**, recomputed after every merge, never merged; absent = no act linked, never a date-only guess. See `docs/decisions/question-au-gouvernement-acte-et-parole-1094.md` |
...
- **`fenetres_parole`** — `{"12_mois"|"6_mois": {debut, fin, nb_membres}}`,
  counted back from **the data date** (generation), the same on every sheet —
  never from `date_reference.date`, which is the closing date of a closed sheet
  (#1081) — and never before `periode.debut`. A closed sheet has empty windows:
  `nb_membres` 0, bounds after its end.
...
- **`mandats_agreges` follows the same bound (#853)**: a committee, friendship
  group or outside body counts only if it overlaps one of the member's
  membership periods — not merely an electoral mandate, which let a 2012
  friendship group into a 2024 group. `nb_membres_cumul_historique` now reads
  « passed through it *while in the group* ».
