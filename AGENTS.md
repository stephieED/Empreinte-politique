...
| Regulatory acts of a government — count, nature, ministry, and **what their title declares of a law** | Public. The link is read in the **title**, never in `liens_lois`: Légifrance barely posts the structured qualification any more (one act out of 19 041 for Lecornu II, against 972 whose title declares it). **« The title does not say so » is never published as « without a law »** — an absence of mention says nothing about the act (§2 rule 5) |
| Share of acts that **apply** a law, or its complement | **Never** — it measures Légifrance's editorial backlog, not the executive's action: 42 % in 2012, 0 % since 2024. → `docs/decisions/part-d-application-non-publiable-1029.md` |
...
| Journal officiel (`echanges.dila.gouv.fr`, DILA) | **Yes since #1029 — regulatory acts only**: decrees, orders (`arrêtés`) and ordinances published since 2007. The source leaves the signatory unpublished, so an act attaches to a government by its publication date and to a ministry by its organe — **never to a person** | Licence Ouverte 2.0 (Etalab) | Attribution only — **no share-alike** |
...
| `docs/sources/` | **External-source references — the only docs that drift with their provider, not with our code.** Update when the provider moves a dataset, a URL pattern or a field, never because our pipeline changed. Each file states its own status in its header (`an-opendata.md`: live, the main French source; `senat-opendata.md`, `parltrack-et-europarl.md` and `jorf-dila.md`: live; `nosdeputes/`: historical, not queried since #529) — the directory names the category, the header names the status. |
...
- `docs/sources/`: external-source references, which drift with their provider and not
  ...
  whole life); `jorf-dila.md` (live since #1029 — the regulatory acts of the Journal
  officiel, and the traps of a fonds whose global dump was delivered once);
  `sycomore-et-journal-officiel.md` (cited, not collected);
